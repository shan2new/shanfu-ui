...
export function AsyncSelectExamples() {
  ...
  const MockPopoverContent = ({ children, style }: any) => (
    <div
      className="absolute top-full left-0 mt-1 rounded border bg-white shadow-lg"
      style={style}
    >
      {children}
    </div>
  ...
  const MockCommandList = ({ children }: any) => (
    <div className="max-h-48 overflow-auto">{children}</div>
  )
  const MockCommandEmpty = ({ children }: any) => (
    <div className="p-2 text-gray-500">{children}</div>
  )
    ...
    <div className="space-y-8 p-6">
      ...
      <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
        <p className="text-sm text-yellow-800">
          <strong>Note:</strong> These examples use mock components for
          demonstration. In your actual implementation, you'll need to pass the
          real shadcn/ui components as props.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">
          Basic AsyncSelect - User Selection
        </h3>
        <AsyncSelect<User>
          fetcher={searchUsers}
          value={selectedUser}
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">
          AsyncCreatableSelect - Tag Selection
        </h3>
