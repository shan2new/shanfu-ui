...
export interface AsyncCreatableSelectProps<T> extends Omit<AsyncSelectProps<T>, 'fetcher'> {
...
export function AsyncCreatableSelect<T>({
  ...
  const enhancedFetcher = useCallback(async (query?: string) => {
  ...
  const shouldShowCreateOption = useCallback(() => {
    ...
    return true;
  }, [allowCreate, searchTerm, isCreating, options, getOptionValue, isValidNewOption]);

  const enhancedRenderOption = useCallback((option: T) => {
  ...
  const wrapperFetcher = useCallback(async (query?: string): Promise<T[]> => {
